...
using Exemplos4.Connection;

namespace Exemplos4.Crud
{
    public class CrudService
    {
        ...
        public List<Usuario> ObterUsuariosPaginado(int pagina, int tamanhoPagina)
        {
            ...
            return _dbConnection.Query<Usuario>(query, new { Limit = tamanhoPagina, Offset = offset }).ToList();
        }

        // Método para retornar a lista de máquinas paginada
        public List<Maquina> ObterMaquinasPaginado(int pagina, int tamanhoPagina)
        {
            int offset = (pagina - 1) * tamanhoPagina;
            string query = @"
                SELECT * 
                 ...
                 ORDER BY ID_Maquina
                 LIMIT @Limit OFFSET @Offset";
            return _dbConnection.Query<Maquina>(query, new { Limit = tamanhoPagina, Offset = offset }).ToList();
        }

        // Método para retornar a lista de softwares paginada
        public List<Software> ObterSoftwaresPaginado(int pagina, int tamanhoPagina)
        {
            int offset = (pagina - 1) * tamanhoPagina;
            string query = @"
                SELECT * 
                 ...
                 LIMIT @Limit OFFSET @Offset";
            return _dbConnection.Query<Software>(query, new { Limit = tamanhoPagina, Offset = offset }).ToList();
        }
    }
}